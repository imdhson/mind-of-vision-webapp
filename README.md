...
src/
  ...
  components/
    ...
    vision/              3D 시각화 탭
    objects/             객체 목록 탭, 상세정보, 선택 객체 시트, 앱 설정
...
- 모델은 객체 목록 탭 → **AI 모델**에서 바꿀 수 있습니다.
...
1. 카메라 화면에서 객체 상자를 누르거나, 3D 화면에서 도형을 누르거나, 객체 목록에서 항목을 누릅니다.
2. 하단 시트에서 **보정**을 누릅니다.
3. 저장 범위를 선택합니다.
...
- 객체 목록 탭 → **저장된 객체 보정값**에서 모든 저장값을 조회·수정·삭제할 수 있습니다.

### 카메라(렌즈)별 보정

객체 목록 탭 → **카메라 설정 · 보정**에서 현재 카메라의 화각(또는 35mm 환산 초점거리), 카메라 높이, 거리 추정 기준 배율,
...
- **Android(Chrome/삼성 인터넷)**: 메뉴 → "앱 설치" 또는 "홈 화면에 추가". 객체 목록 탭 → 앱 → "홈 화면에 앱 설치" 버튼도 지원.
...
- WebGL 이 없는 환경에서는 CPU 로 추론하므로 매우 느리고, 3D 탭은 표시되지 않습니다(객체 목록으로 확인 가능).
...
객체 목록 → 새로고침 후 보정값 유지·자동 연결 안 함 → Service Worker·Manifest → 다크 모드 → 모바일(터치 선택,
...
| # | 절차 | 기대 결과 |
| --- | --- | --- |
| 1 | PC(Windows 11, AMD 내장 그래픽)에서 `npm run dev` → Chrome 으로 접속 → 카메라 시작 | 영상 표시, 헤더에 "N개 · fps", 객체 목록 탭의 연산 백엔드 `webgl` |
