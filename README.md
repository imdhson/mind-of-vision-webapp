...
| 차선 인식 | **차선(도로 경계) 인식**(추가 모델 없음, 경계 검출 + Hough 직선): 좌/우 차선·도로 가장자리를 카메라 화면에 파란 선과 주행 영역으로 표시하고, 카메라 높이·기울기로 3D 바닥에 투영. 카메라 화면 버튼/설정 탭에서 켜고 끔 |
| 방향 | 이동 방향(속도 회귀)과 바라보는 방향을 구분. 사람·동물·탈것은 이동 시 진행 방향으로 추정, 정지 물체는 "미확인" + 사용자 지정 |
| 3D 시각화 | 미니멀 3D 공간, 원형 사용자 마커(텍스트 없음), 연한 바닥 격자·거리 링, 객체 종류별 형상, 회전·확대·핀치, 기본 시점 복귀, **매우 느린 자동 확대·축소**, 3D 객체 클릭/터치 선택(Raycasting), **이동 궤적**(움직이는 객체 아래 최근 경로를 옅은 선으로 표시) |
...
| 안전 알림 | **근접 경고**(설정 탭에서 켜기/거리 지정): 추적 중인 객체가 지정한 거리보다 가까워지면 토스트 알림 + 짧은 알림음 + 진동(지원 기기)으로 안내. 같은 객체는 구역 안에 머무는 동안 쿨다운을 두고, 벗어났다 다시 들어오면 즉시 재경고 |
...
src/
  ...
  features/
    ...
    visualization/       VisionScene, ObjectRenderer, UserMarker, shapes, AutoCameraController, TrailTracker(이동 궤적)
    lanes/               LaneDetector(경계 검출·Hough 직선), LaneTracker(시간 안정화), laneGeometry(바닥 투영)
    safety/              ProximityMonitor(근접 경고 판정), proximityRuntime(구독·토스트·소리·진동 연결)
...
3D 화면에서 움직이는 객체 아래에 최근 이동 경로가 옅은 선으로 표시됩니다.

- 정지한 객체(위치가 거의 변하지 않는 객체)에는 표시되지 않습니다. 추정치의 미세한 흔들림으로 궤적이 남는 것을
  막기 위해, 일정 거리(기본 0.15m) 이상 이동했을 때만 점을 추가합니다.
- 궤적은 최근 약 6초 · 최대 40개 점까지만 유지되며, 추적이 끝나면(객체가 목록에서 사라지면) 즉시 함께 사라집니다.
- 저장되지 않는 일시적인 시각화이며, 보정값처럼 IndexedDB 에 남지 않습니다.

### 근접 경고

설정 탭 → **안전 알림**에서 켜면, 추적 중인 객체와의 추정 거리가 지정한 값(기본 1.5m, 0.3~10m)보다 가까워질 때
토스트 알림 + 짧은 알림음(WebAudio, 별도 파일 없음) + 진동(지원 기기)으로 안내합니다.

- **확정된 추적**(연속 인식으로 확정되었거나, 잠시 가려진 상태)만 대상이며, 막 인식되기 시작한 객체나 추적이 끝난
  객체는 경고하지 않습니다.
- 같은 객체가 구역 안에 계속 머무르면 약 4초 쿨다운을 두고 반복 경고하며, 구역을 벗어났다 다시 들어오면 쿨다운과
  무관하게 즉시 다시 경고합니다.
- 거리 계산은 8절의 단안 거리 추정을 그대로 사용하므로 **참고용**입니다. 정확한 안전 거리 판단에는 사용하지 마세요.
- 설정값(켜짐 여부·거리)은 IndexedDB 에 저장되어 재실행 후에도 유지됩니다.

### 차선(도로 경계) 인식

카메라 화면 위쪽의 도로 모양 버튼 또는 설정 탭 → **도로 인식**에서 켜고 끕니다(기본 켜짐, 설정은 IndexedDB 에 저장).

- 별도 AI 모델 없이 영상 처리로 동작합니다: 객체 인식과 같은 주기로 축소 프레임(너비 256px)을 만들어
  명암 → 흐림 → Sobel 경계 → Hough 직선 변환으로 앞쪽으로 뻗은 좌/우 경계선을 찾습니다.
- 카메라 기울기(피치)로 계산한 **지평선 아래**만 검사하며, 거의 수평인 경계(그림자·보도블록 이음새 등)는 제외합니다.
  흰/노란 차선뿐 아니라 아스팔트와 잔디·연석 사이 같은 **도로 가장자리**도 경계로 인식합니다.
- 연속 2회 이상 비슷한 위치에서 인식되어야 표시하고, 점선 차선의 빈 구간처럼 잠시 놓치면 약 0.7초 동안 유지합니다.
- 카메라 화면: 좌/우 경계를 파란 선으로, 두 선 사이 주행 영역을 옅게 채웁니다(두 선이 만나면 소실점 직전까지만).
- 3D 화면: 카메라 높이·기울기로 경계선을 바닥에 투영해 최대 30m 까지 그립니다(카메라 보정값이 정확할수록 폭이 정확).
- 직선 모델이므로 급한 커브는 가까운 구간의 접선으로 표시되며, 실내나 직선이 많은 장면에서는 다른 경계를 차선으로
  잘못 표시할 수 있습니다. **참고용 시각화**이며 주행 보조 용도로 사용하지 마세요.
...
재경고·확정 전/종료된 추적 제외), 차선 인식(합성 도로 영상에서 좌/우 경계 검출·수평선/관심 영역 밖 직선 제외·
지평선 계산·연속 인식 확정·유지·급변 억제·교차 처리·바닥 투영).
